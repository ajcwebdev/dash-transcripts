...
[00:20] refilled with dash um and that's what we're paying for the stream yard with so we've got that up to date and um yeah we've got aj back with us
...
[00:46] that uh he has a current uh current proposal up for four dash to maintain that service so check that out if you haven't seen it already from last week
...
[01:08] not the kind of trpc that you're thinking of anthony because i know that you're thinking of typescript rpc when i say you know how much i'm thinking of getting
...
[01:37] browser and bringing those all together we've got um we've got coin join working in the browser do you want to give us a little uh just description of what
...
[01:57] window here we go we're gonna we're gonna share it all right so here we are and what basically you've got your your faucets you've got all this wallet information
...
[02:27] here whoops you got your wallet information here um you've got information about coins that you currently have here you have the ability to send to an address to send
...
[02:48] a cash drawer which we'll talk about that later and this is basically targets for for what we want so let's say i actually don't want any of these i'm
...
[03:11] addresses unused change addresses spin addresses let's say i just want to denominate some coins let's say that i want uh five of these so i'm going to just do
[03:18] denominate coins all right are you sure yes yes yes oh and by the way just so you know we we don't see those um alerts that you're oh you
...
[03:36] pop-ups because it was simpler than you know i just i just went simple like i didn't do any styling on this this is a free theme that i i
[03:42] just like because it's fairly simple and clean so i use it a lot so
[03:48] i didn't actually create a modal and the theme doesn't have a specific modal in it you can do a modal with it it's just it's not there's not a
[03:55] modal thing anyway using the browser alert which we don't see yeah maybe when you
[04:00] when you do come up with those just say what they say yeah so it says are you sure you want to send uh what was it 0.012 and then
...
[04:20] to our coins you can see that these are bolded and italicized which means that they are denominated and then we'll do a demo a little bit later or let's
[04:26] just do the demo now actually we'll just do the demo now i just have
[04:31] to open up in two browsers in order for it to work actually um so i'm going to actually share my screen because i want to show something other than
...
[04:51] just bring this up so you guys aren't seeing the infinite thing there um and then i'm going to bring up my test net node if you're wondering i've increased
...
[05:11] you can see can you see my my little window popping up here yeah yes okay so you can see this is orange that means uh it's a test net
[05:18] you can also see this is test dash right here down here um so i
[05:24] haven't done this yet ever on this wallet so i'm going to run the coin process on you this is not going to work the way you think okay you
...
[05:45] work you have to connect to a node where someone else is advertising the same denomination that you want to join on the way that it's done in my demo
...
[06:12] denomination of coin if your computer picks one it randomly and my browser picks one randomly and we're on test net where there's just not as many pools available and
[06:20] then we both try to and and my browser demo right now is only opening
[06:28] up one pool whereas the desktop is opening up 10 pools at least from what what we saw the other day it looks like and so the desktop is and
[06:35] the desktop has been running in the background for a while so it already has
[06:42] its pools built up whereas the browser is not okay well the browser is not built to run 10 pools at a time right now it can it's just i
[06:48] don't have that functionality in because it was you know the the mvp demo that
[06:56] i was aiming for so sure it it can do you can do the coin join between the desktop and the browser it's just that if you want to do
...
[07:12] that you know already has the pool that you want and when you listen there will be a table for this what do you mean just just just tell people
[07:18] what you mean by pools so there's a message called dark send queue and the
[07:28] dark send queue message is broadcast from master node that you're connected to and for every coin join round that is in the um what what would we call that
...
[08:25] that you're interested in joining of the the five possible denominations is actually decently high um i'm not sure if that holds on testnet as well as it does on
...
[08:42] hundreds or thousands of times at this point but anyway um yeah so there's there's that aspect to it where the in in the browser demo we're only connecting to
...
[09:05] open and even if it's not the pool you need most right now it's going to be a pool that you can use on some denomination of coin until you've
[09:13] hit your your cash drawer limits which the cash drawer is not ex the cash
[09:22] drawer exists in um in dash qt the desktop client but it's not exposed there's no way for you to access it okay so i was actually just going to
...
[09:49] you got your target balance this these are all these were all the defaults i haven't changed anything about these so it's interesting that the default inputs per denomination is
[09:54] targeting 50 and giving a maximum of 300. i'm going to bump that way down
[10:06] to 20 and 10. this is basically you did 50 and 10. say say what you did 50 and 10. i don't know why but um okay so let's do
[10:15] 20 and 10 and i'm just going to go ahead and start this um i
[10:24] have something like 10 i have like 40 dash i think of balance in my uh in my wallet so i guess go ahead and say yes i might be
...
[10:54] and it's not always the denomination that i want so if i select 0.01 and a q comes through for 1.0 and 0.1 i can't use it and so one
[11:06] of the updates that will be coming i believe this week to this tool will be to put the dark send qs in a table so that uh it's easier
[11:17] to select and then also uh collect connecting to multiple master nodes at a time
[11:26] so that there are many queues available okay so i i'm just um i i just kind of wanted to get this on screen just for out of out of
[11:34] out of out of curiosity i'm gonna open my debug.log um and i i don't
[11:46] know i guess probably nothing on here is private but i don't know how do i zoom in on this uh i guess i i don't know if i can
...
[12:20] if i will have any kind of mixing partners right now and then when you join in if it would speed things up so you're saying it won't connect to
[12:26] my the nodes that i'm connecting to but if you if you can give me
[12:32] a list of nodes you're connected to and and yeah if you could that's what i was doing here on my peers i don't know do you know the ip
[12:42] address of your um so you're connecting through rpc your rpc service right and you
[12:54] got 35 90 252 3 because i could so all of these are dcg ones that say dcg so it'd be just the ones that don't um i'm assuming i
...
[13:21] go through this i thought it might just be interesting um to see some of the to see if we could connect to each other and and and see that
...
[13:46] we can yeah we can definitely get it later so that it works um for that use case okay i'm going to share the screen all right so what i
...
[14:06] so what you can see there's these dark send q messages that come through and then i have here printed out what denomination that q is for and so the
...
[14:37] so there's two wallets i've got one blue and one purple they're in different browsers because if you have shared state between the wallet and you try to open up
...
[14:57] you know things are the way that they they're supposed to be and if if the wallet if the wallet is in an inconsistent state because it's opened up in
...
[15:14] see there's some dark send q messages going through and they come through a different amount so there's the smallest one right there you can kind of count the zeros
...
[15:34] 000 i don't remember but anyway so you can see they're coming through in different denominations this is actually you know by random chance this is a really good node
...
[15:51] open for the other browser so that i can see that both are connected and working so anyway i can go through here and let's see what have i got
[15:59] 0.01 so i'm going to select two of these over here and i'm going to
[16:06] select run coin join and then wait wait before you do that have you done have you done any coin joining yet or just the denominating what have you done
...
[16:40] this uh network yes i guess yes so once i click the button i kind of have to click the other button otherwise it's gonna time out so i just
...
[16:58] other browser and it already completed it already worked out so there's a bunch of dsq messages broadcast right as you started speaking i was going to point out that
...
[17:15] brought that browser is causing to announce or re-announce okay and so that dsq for the amount had gone through along with other dsqs that are happening on the network
...
[17:36] the collateral is a type of memo uh in most cases you can actually do the collateral in a bunch of different ways at least not according to the spec
...
[17:57] it's not just the dash core client they would probably follow the spec anyway um so yeah so you can see that i've got my collateral and memo signed messages
[18:03] and then the dsa comes in so i'm sending the dsa that's what i'm announcing
[18:09] a dssu comes back to let me know that i've been accepted into a pool and that happens immediately so that means that the pool is already open so this
...
[18:36] the whole time because that's whatever's going on on the network some more dssus uh and then we have the dsf which is it's really weird it's called the dark
...
[18:56] transaction and send it in or rather you create your you create a partial transaction so a transaction with partial signatures and you send that in so that's what the
...
[19:16] at this uh let's see does it actually show the addresses we could we could see we could convert these pubkey hashes into the and find out which addresses are
...
[19:32] the inputs that i can sign and which of these outputs or outputs that i can sign and then i'm going to send back a message that includes partial signatures
...
[19:56] i send a message called dss which is dark sense i think it's dark send signed and then we get back a message dsc which is dark send complete and
[20:03] so that lets us know that it was it was already processed in the queue
[20:10] and then if we were to look at these addresses so i could go look at these these addresses here um let's let's look at the rpc tool real quick
[20:18] and then we'll do uh get address deltas and then oops address i think it's
[20:38] addresses addresses address oh bless me either way all right so now we'll run the rpc and we can see the output and you can see exactly what you'd expect
...
[21:36] a coin join transaction another thing that you can that will let you know it's a coin join transaction and not some you know magic trickery that i'm doing to
...
[21:57] submitted via a side channel now you know it's kind of tomato potato because the collateral still ends up in the same block so it's not like it's not linked
...
[22:21] i could pay you any time in the week i could set up that payment and i could use the payment to you as a collateral transaction it doesn't it
...
[22:45] can still figure out which transaction was the collateral related to um you know coin join sessions you can't tell which of the coin join sessions and the block it's
...
[23:11] in in the insight and yeah yeah sure so let's let's go to insight here whoops just so that people can see that with the more familiar uh interface anthony
...
[23:44] insights on life support and test net insight has not quite made life support priority you're on the dash evo domain and that's not yeah that's not the one that's
...
[24:07] um let me see if i've got it on digitalcache.dev whoops because i did include yeah let's see this one it's this one insight ui here okay yeah on on
...
[24:47] okay so so anyway but yeah you were able to see this we could have put this in a table and and you know you'd be able to see it
...
[25:06] from here but this is this is simple debugging tool uh yeah so that that uh unfortunately took a little longer than i was hoping but it does the business
...
[25:34] i'd recommend using words instead of numbers for priority like highest versus 10 because there's already so many numbers to look at when dealing with the denominations just one little
...
[25:52] one is the highest priority so maybe if you did it like a's and then translated those into uh numbers on the back end that oh yeah it it really
[25:59] is not that big of a deal i really like the idea of using um
[26:10] the the alphabet the reason for numbers is that numbers there's an infinite number of numbers so if you decide that okay i want to make i actually want to
...
[26:27] what i want to do is i want to make i want to make this coin the highest priority because i'm debugging then all i have to do is increment
[26:34] that you know i all i have to do is make that number the biggest
[26:40] number and then now it takes the highest priority when i click the button but yeah i i there there is only a i guess the better way to do
[26:48] it might be to have the like have the columns move up and down or
[26:56] something but but yeah the the i i think that's a great idea it's a great suggestion and there's so few of these that you don't need infinite numbers it's
[27:01] just nice for me because i don't want to say well i had this at
[27:07] highest but now i want to make this highest so how do i make it highest er okay um most highest yeah yeah but then you need most or highest
...
[27:48] do you think the pros and cons would be of adding newer uh additional denominations so right now coin join has denominations of 0.0001 0.1 1 1 10 and i
[28:02] think that's it so there's like all they're all orders of magnitude of of 10. now i know that whenever i almost almost 100 of the time i would say uh like not quite that much but 90 plus percent of the time that i
...
[28:47] said you know you don't need to worry about that or whatnot um but the overall statement i think is true the more coins you're using the less the less
...
[29:30] between mathematical perfection and reality is that the optimal number of bits of information is the number e which is 2.7 something something something e is a mathematical constant it's
...
[30:05] mathematical perfection and reality kind of don't mix because e is not a countable number it's not like one two e three you know it's one two three and so
[30:13] you round to three so in practical sense three is the most perfect um medium
[30:24] to store information now let's bring that back to reality it's the rule of threes right we have all these things that are three we have uh the in religion
...
[30:58] kind of a magic number in terms of choices and trade-offs that is just kind of appeared in humanity that's just something very natural and this has to do uh
...
[31:34] also exists although you might not have recognized it humans are built such that they understand orders of magnitude right so i need to understand is this group of tigers
[31:41] bigger or smaller than this other group of tigers and and by what order of
[31:48] magnitude is it like twice as big or is it 10 times as big i can't tell you that there's 100 here and there's 357 there by looking but i
...
[32:09] depending on what you consider to be order of magnitude whether you're doing two like binary and computers or 10 like people often do uh you know you you can
[32:17] you can get a sense of scale even though you can't count and so in
[32:26] our money our money has become three choices for every order of magnitude so you have your i mean it's not perfectly true in all situations but it tends this
...
[32:56] it's actually representing the orders of magnitude even better than you think when you count this way but then it's 10 20 50 we used to have a 50 bill
...
[33:21] we also were inventing credit card systems check systems digital cash systems you know so we we didn't we didn't keep going with the alternate explanation to that is that
...
[33:42] was our number system and if you go down it's 50 and then this is where it gets a little weird because we have a 25 and a 10 but
...
[34:02] silver nickel copper um i think that the reason that it breaks down at the coin level has more to do with the the way that the metals were processed
[34:08] than that it was the you know the the natural choice but yeah then you
[34:14] know you go down one and we used to have a tuppence we don't have a tuppence anymore but we had one two five but then dimes were silver and
[34:21] so it was a tenth of a silver dollar and i i think quarters were
[34:28] silver as well and then we had the half dollar which would be the five again so but so what this actually is is it's doubles holes and halves so
...
[34:46] as your starting point 20 is your double and then five dollars is your half so this is the pattern that appears in i want to say in in nature
...
[35:18] an order of magnitude so one double is two half is 0.5 tens double is 20 half is five hundreds double is 200 half is 50. yep and if we
[35:32] take that now if you share my screen one of the problems that you saw
[35:39] with the cash drawer in dash core that implementation of the cash drawer is that it did not give you any um option to deal with the scale of the
[35:47] money which is actually really important because you know you think about a lot of
[35:57] kids may not get this but the adults um you know will when you used to have money in your wallet what did you have you had one 100 you
[36:05] had two or three 20s you had a handful of fives and several ones right
[36:15] because you organize your wallet your cash drawer or like the reason i call this cash drawer is because it's the cash drawer at the registry you know or register
...
[36:34] have in and and uh you know paper money but the the system uh is that you don't you don't make lots and lots of room for the hundreds because
...
[37:06] bills more often than you're using pennies right so this option is the one that you almost want none of if you can ever avoid it this one doesn't really
...
[37:25] we were to look at this this represents about two and a half dollars you know so you need a few of these to make up the difference and and
...
[37:44] curve where i have something like 25 um you know obviously it depends on on the size of transactions that you're typically doing sure yes um and this comes back
[37:50] to the back to the issue that i was raising where sometimes you're doing it
[37:57] uh almost always i i get that warning that i'm doing um i'm spending too many coins and that would you know there's toggles that i can i can change
...
[38:19] a transaction i'm doing so if i'm doing a transaction where i need 20 uh you know to send 20 dash and my drawer doesn't have any tens i'm going
...
[38:43] most people are going to be transactions that are based primarily on the 20 dollar amount you personally because of what you do i think i think you actually still
...
[39:07] coin join transactions for for incubator but um i think the the point is if if we had more denominations to choose from if we had 20s if we had
...
[39:32] to be a logarithmic so this is mathematically perfect you cannot design a better system not this but what i just described you cannot build a better system than that
...
[40:01] introducing more denominations the twos and the fives i know that one one um concern about that is that the more denominations you add the less uh likely it is
...
[40:24] that a little bit yeah it's not it's not the trade-off that you think it is right because again there's a bell curve here right so if the way that
...
[40:45] yes that that problem would persist so if we introduced more denominations and we did not adjust at all the way that we cash drawer the denominations then yes that
...
[41:23] have to break the mathematical rules in order to do a regular transaction so where if you put it in a bell curve most of the time you're going to
...
[41:52] you're always getting 50 of each but that doesn't that doesn't mat that matches what you have it doesn't match what you need yeah so and i i know that
...
[42:11] more about the coin join stuff than him in the dash community and he's been pushing for an implementation like this where you in the actual core wallet you can
...
[42:36] i wonder so is this something that you think would be a priority like do you want to see the introduction of new denominations and i do know that uh
...
[43:29] worlds where uh even like normal transactions that aren't coin join transactions they would be more private they would be uh more secure is the term that you like to
...
[43:49] send is and um how this relates and and what the path might be to okay getting from here to there so there's two components there's coin join and there's
...
[44:07] i want to describe that a little bit in in the way that i understand it so when you look at a block explorer and you see a coin when
...
[44:28] terms like the way like we're using them i think that we're using them in a way that's more descriptive because a lot of times people think coin join is
...
[44:53] we have had this transition where we had a term called private send and then in the ryan taylor era of dash core group there was a push to d
...
[45:17] this push to just say okay well the industry is using this term coin join which is the technology that private send uses mostly um and so let's unbrand private
...
[45:37] a different branding of private send but i would like uh and i and you would agree i'm sure i would like us to have a more uh defined distinction
...
[46:10] what we are calling a coin join transaction and private send would be when you take the outputs of a coin join transaction and then spend them on say something
...
[46:33] uses it uses two tens and five ones so seven coins of of inputs and it has let's say one out sorry sorry were you talking about cash send or
[46:38] are you talking about private send i'm talking about private send oh sorry yes it
[46:43] does come i'm not at cash send yet uh i'm talking on this so that it's clear so and a private send transaction which is the the term that the
...
[47:04] the next transaction and the output of that transaction is let's say 27 dash and that uses uh two plus seven so nine coins to do that and the output
...
[47:38] more than 10 coins because when you combine so many coins you can't determine specifically which coin it came from but what you can tell is the probability of a
...
[48:00] likely that it came from these three coins you know well and what it definitely tells people is that these these nine coins definitely belong to somebody that has control
...
[48:40] have multiple outputs that are um it's not one output it's multiple outputs there you go yeah there's different terms we're flirting with in the beginning okay so now now
...
[49:14] a private send with cash send a coin join transaction is a cash send transaction and a private send transaction is a cash send transaction so cash send is that
...
[49:45] the value and there's the segment that represents the fee and so the first three zeros after the decimal place represent the value and the next five zeros after the
[50:00] decimal place represent the fee and that is that is a programmatic something that could be changed but it turns out that with our current fee structure that's the best
[50:05] way to do it uh for a variety of reasons and as long as dash
[50:12] stays under a thousand dollars that still works because the reality is when you do an exchange from francs to euros to dollars you're going to lose a few pennies
...
[51:12] down that it didn't round perfectly to okay i got 94.9978 you know like that that's not the way that that's not the way that you expect it you're okay
[51:19] with the fact that it rounds down and the person that's facilitating the transaction gets the bits that were rounded down in in their account essentially so that's that's one
...
[51:54] useful and the bottom portion the dust is denominated for fees in orders of 200 and so the way that the way that it works is you've got your uh
...
[52:21] to a lot of nodes and wait a long time but you're also probably not going to be spending it right away so you know there there's that um maybe
...
[52:47] so few mixes that it wouldn't do you any benefit at the current time um but that would just happen naturally i mean that that wouldn't even be a thing
...
[53:16] times more than the current number of denominations but like i said with the bell curve what you're going to be tending towards you know is the most somewhere in
...
[53:36] to define how quickly you can get those done um and then let's see if i if i had like another table or anything and then yeah the trailing the
...
[53:58] sending the denominations stay the same and the fees are slightly adjusted whatever the cost of the transaction is the fees are re-averaged across the coins yeah um and so
...
[54:20] i just wanted to give people a a brief um introduction reintroduction to that concept of cash tokens or cash cash send not cash tokens cash the key thing the
...
[54:51] wallets for the transaction so that's that's the other key thing right because you would give somebody your your um x pub for them so if if jojo and i
[55:03] are contacts i have an x pub that represents jojo jojo has an x pub
[55:09] that represents me whenever i need to send to jojo i'm not re-scan i only scan his qr code the one time that i create his contact any other time
[55:17] i need to send to jojo i just select him from the list and then
[55:22] my wallet is going to pick addresses that haven't been used yet and they're going to send you know the one the five the 0.02 it's going to send the
...
[56:11] join transactions with cash send person-to-person transactions so you get like quadruple the benefits because even within the transaction you don't know which coins are a payment you don't even
...
[56:33] and i are going we've been going down this road in our heads for over a year now we haven't really we have done those dips uh but the first
...
[56:54] can do you can implement the dip without ever touching anything in in that layer of the code the the dip can be implemented all the way up at the
...
[57:18] the denominations yes which is the topic of today's uh you know that's that's the topic today so that's why i was bringing it up as well uh so i
[57:23] think we've gone for almost an hour um is there anything else that you wanted
[57:30] to talk about uh your let's go back to the the tool again the the web the web um coin join interface anything else that you wanted to say about
[57:38] this plans moving forward um before we wrap up today i you know i don't
[57:47] i don't really know i i need to take a breather for a day or two uh from it because i i did a lot of high intensity work on
[57:55] it and uh you know for those people that do knowledge work you know how
[58:00] it goes and people who do physical work they know how that goes too because it's the same thing you know like you you do a real big push to
[58:04] meet any sort of goal or deadline and you need a couple days to recuperate
[58:10] we got a great little meme i share with my wife or it says that was fun now i'm gonna take a 21 hour nap yeah yeah yeah and that
[58:18] and that's that's kind of how i feel but um i want to get the
[58:25] cues i want it to get so that you can see which cues are available so like maybe put a little asterisk by the coins for which if you choose
[58:31] those coins you'll be able to mix them right away so as cues come in
[58:37] and cues have i don't really i have to go look up in the spec what the lifetime of a queue is because it times out after i don't know
...
[58:56] people might be wanting to know is when when is this going to support mainnet um oh it sure it supports mainnet as soon as i flip the bit but
[59:03] you know i've got the word for it i need to blank that out you
[59:10] know put hide that under when a little lock icon in a menu or something yeah and even when it does go to mainnet this will definitely still be like
...
[59:29] this this is fully tested it goes by the spec if a transaction goes on the blockchain it is there there is no like because the ui is ugly the
...
[59:49] no this is production code this is not alpha code when we talk about the ability to do the coin processes so like i wish you would stop saying that
...
[60:07] audience all right i'm saying to the audience do not use this code even though it's even though what despite what aj said if you are not comfortable with coins
...
[60:33] cache but the browser application cache which is separate from the normal browser history cache okay fair enough all right so with that i think we'll say goodbye thank you
[60:39] everybody for joining in if you have any questions drop them in the comments and we will get back to you